...
const CreateTask = (props) => {
  ...
  return (
    <div className="py-8 md:py-10 bg-app-bg min-h-screen relative overflow-x-hidden transition-colors duration-300 font-sans">
      {/* --- TOAST NOTIFICATION --- */}
      <div
        className={`fixed top-10 left-1/2 -translate-x-1/2 z-[100] transition-all duration-500 transform ${showToast ? "translate-y-0 opacity-100" : "-translate-y-20 opacity-0 pointer-events-none"}`}
      >
      ...
      <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-10 md:gap-12 px-3 sm:px-6 md:px-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-black text-text-main tracking-tighter leading-tight uppercase">
            Forge <br />
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-600 to-cyan-600 dark:from-emerald-400 dark:to-cyan-400 text-6xl">
              New Task
            </span>
          </h1>
        </div>
            ...
            <form onSubmit={submitHandle} className="space-y-8">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-text-sub ml-1 text-xs">
                    Objective Name
                  </label>
                  <input
                    value={taskTitle}
                ...
                <div className="space-y-2 relative" ref={calendarRef}>
                  ...
                  {/* Calendar Dropdown UI - Fixed for Mobile */}
                  {showCalendar && (
                    <div className="absolute z-50 top-full mt-3 left-0 right-0 md:left-auto md:right-0 md:w-80 bg-card-bg border border-border-subtle rounded-[2rem] md:rounded-[2.5rem] shadow-2xl p-4 md:p-6 backdrop-blur-xl animate-in fade-in zoom-in duration-200 mx-auto">
                      {/* YEAR NAV BADGE */}
                      <div className="flex justify-center mb-4">
                        <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full flex items-center gap-4 transition-all">
                          <button
                            type="button"
                            onClick={() => changeMonth(-12)}
                            className="text-emerald-500 hover:scale-125 transition-transform text-xs font-bold px-1"
                          >
                            ‹
                          </button>
                          <span className="text-[10px] font-black text-emerald-600 dark:text-emerald-400 tracking-[0.2em] tabular-nums">
                      ...
                      <div className="flex items-center justify-between mb-4">
                        <button
                          ...
                          className="p-2 hover:bg-emerald-500/10 rounded-full text-emerald-500 group"
                        >
                          <span className="group-hover:-translate-x-0.5 transition-transform inline-block">
                            ←
                          </span>
                        </button>
                        <span className="font-black uppercase tracking-widest text-[11px] md:text-[13px] text-text-main">
                      ...
                      {/* Days Grid */}
                      <div className="grid grid-cols-7 gap-1 mb-2 text-center text-[9px] md:text-[10px] font-black text-text-sub opacity-40 uppercase">
                        ...
                        {Array.from(
                          ...
                          return (
                            <div
                              ...
                              className={`py-2 text-xs md:text-sm font-bold rounded-xl cursor-pointer transition-all ${isSelected ? "bg-emerald-500 text-white shadow-lg shadow-emerald-500/20" : "hover:bg-emerald-500/10 text-text-main"}`}
                            >
                              {day}
                            </div>
                          );
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="space-y-2 relative" ref={dropdownRef}>
                  ...
                  <div
                    ...
                    className="w-full bg-surface dark:bg-white/5 border border-border-subtle rounded-2xl py-4 px-6 text-text-main flex items-center justify-between cursor-pointer hover:border-emerald-500/50 transition-all"
                  >
                  ...
                  {isOpen && (
                    <div
                      className={`absolute z-40 w-full bg-card-bg border border-border-subtle rounded-2xl shadow-2xl max-h-48 overflow-y-auto p-2 custom-scrollbar ${dropdownDirection === "up" ? "bottom-full mb-2" : "top-full mt-2"}`}
                    >
                      {userData.employees.map((elem, idx) => (
                        <div
                          ...
                          className="flex items-center gap-3 px-4 py-3 hover:bg-emerald-500/10 rounded-xl cursor-pointer transition-all"
                        >
                          <div className="h-8 w-8 rounded-lg bg-emerald-500/20 flex items-center justify-center text-emerald-600 font-bold text-xs">
              ...
              >
                ...
                <span className="relative z-10 uppercase tracking-[0.4em] text-xs group-hover:text-white transition-colors duration-200">
                  Deploy Objective
                </span>
              </button>
            </form>
